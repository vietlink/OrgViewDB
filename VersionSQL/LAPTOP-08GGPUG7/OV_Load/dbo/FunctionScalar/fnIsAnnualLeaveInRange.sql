/****** Object:  Function [dbo].[fnIsAnnualLeaveInRange]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- Description:	
-- =============================================
CREATE FUNCTION [dbo].[fnIsAnnualLeaveInRange] 
(
...
BEGIN
	...
	IF @nextAnnualLeave IS NULL 
		RETURN 0;
	IF (@nextPublicHoliday IS NULL AND @nextNWD IS NULL) 					
			RETURN 1;
	ELSE IF (@nextPublicHoliday IS NULL AND @nextNWD IS NOT NULL) BEGIN
		IF (@nextAnnualLeave< @nextNWD) RETURN 1; ELSE RETURN 0;
	END ELSE IF (@nextNWD IS NULL AND @nextAnnualLeave IS NOT NULL) BEGIN
		IF (@nextAnnualLeave< @nextPublicHoliday) RETURN 1; ELSE RETURN 0;
	END ELSE BEGIN
		IF (@nextAnnualLeave<@nextPublicHoliday AND @nextAnnualLeave<@nextNWD) RETURN 1;
		ELSE RETURN 0;	
	END	
	RETURN 0;	
