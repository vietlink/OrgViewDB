/****** Object:  Function [dbo].[uspGetTotalHeadCountRecursive]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE FUNCTION [dbo].[uspGetTotalHeadCountRecursive](@PositionId int)
RETURNS int
AS
...
DECLARE @VacantCount int = 0;
 ...
 WITH RPL (Id, ParentId, EmpPosId,employeeid,ExclFromSubordCount,EpIsDeleted,PIsDeleted) AS
  ...
  SELECT @EmpCount =(select count(distinct RPL.employeeid) FROM RPL INNER JOIN Employee e ON e.isdeleted = 0 and e.id = RPL.employeeid INNER JOIN [Status] s on s.[Description] = e.[status] and s.IsVisibleChart = 1 WHERE e.Identifier <> 'Vacant' AND RPL.ExclFromSubordCount='N' AND RPL.EpIsDeleted = 0 AND RPL.PIsDeleted = 0),
	@VacantCount =(select count(distinct RPL.EmpPosId) FROM RPL INNER JOIN Employee e ON e.isdeleted = 0 and e.id = RPL.employeeid WHERE e.identifier = 'Vacant' AND RPL.ExclFromSubordCount='N' AND RPL.EpIsDeleted = 0 AND RPL.PIsDeleted = 0);
