/****** Object:  Function [dbo].[fnIsAnnualLeaveBeforeNonWorkDay]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE FUNCTION [dbo].[fnIsAnnualLeaveBeforeNonWorkDay](@empId int, @date DateTime)
RETURNS bit
AS
BEGIN

	-- Find the last annual leave in a 2 week range
	DECLARE @lastAnnualLeave DateTime;
	SELECT 
		@lastAnnualLeave = MAX(lrd.LeaveDateFrom)
	FROM 
		LeaveRequest lr
	INNER JOIN 
	...
	DECLARE @StartDate DateTime = DATEADD(day, 1, @lastAnnualLeave);
