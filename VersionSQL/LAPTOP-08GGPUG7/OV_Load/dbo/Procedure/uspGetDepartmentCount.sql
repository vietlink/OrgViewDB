/****** Object:  Procedure [dbo].[uspGetDepartmentCount]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[uspGetDepartmentCount](@date datetime, @listDivision varchar(max), @listDepartment varchar(max), @listLocation varchar(max), @listEmployeeStatus varchar(max), @listEmployeeType varchar(max))
AS
BEGIN
	...
	SELECT 
		rs.department,
		count(rs.department) as [count] FROM 
			(SELECT 
		...
		AND ((SELECT COUNT(*) FROM @locationTable) = 0 OR case when isnull(e.location, '') = '' then '(Blank)' else e.location end IN (SELECT * FROM @locationTable))
		 ...
		 GROUP BY rs.department
		 ORDER BY rs.department
END
