/****** Object:  Procedure [dbo].[uspAddUpdatePositionEntryGroupRelations]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[uspAddUpdatePositionEntryGroupRelations](@groupId int, @idList varchar(max), @mandatoryList varchar(max))
AS
BEGIN
...
--	SET @outputTable = fnSplitString(@idList, ',');
	...
	DELETE FROM
		PositionEntryGroupRelations
	WHERE
		PositionEntryGroupID = @groupId
	AND
	...
	WHILE @@FETCH_STATUS = 0 BEGIN
		...
		IF NOT EXISTS (SELECT attributeid FROM PositionEntryGroupRelations WHERE PositionEntryGroupID = @groupId AND AttributeID = @insertId) BEGIN
			INSERT INTO
				PositionEntryGroupRelations(PositionEntryGroupID, AttributeID, IsMandatory, SortOrder)
					VALUES(@groupId, @insertId, 
					CASE WHEN EXISTS (SELECT id FROM @mandatoryTable WHERE id = @insertId) THEN 1 ELSE 0 END,
					ISNULL((SELECT MAX(SortOrder) + 1 FROM PositionEntryGroupRelations WHERE PositionEntryGroupID = @groupId), 1))
		END
		ELSE BEGIN
			UPDATE 
				PositionEntryGroupRelations
			SET 
				IsMandatory = CASE WHEN EXISTS (SELECT id FROM @mandatoryTable WHERE id = @insertId) THEN 1 ELSE 0 END
			WHERE
				PositionEntryGroupID = @groupId AND AttributeID = @insertId
		END
