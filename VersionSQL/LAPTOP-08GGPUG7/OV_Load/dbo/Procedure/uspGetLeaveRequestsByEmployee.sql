...
BEGIN
	...
	DECLARE @StartDate DATE = DATEADD(month, -14, GETDATE());
	DECLARE @EndDate DATE = DATEADD(month, 14, GETDATE());
	...
	WHERE
		r.IsCancelled = 0 AND
		r.EmployeeID = @empId AND r.id <> @excludeId AND ewh.EmployeeWorkHoursHeaderID = r.EmployeeWorkHoursHeaderID 
		AND ewh.[week] = dbo.fnGetWeekByHeaderDate(rd.EmployeeWorkHoursHeaderID, rd.LeaveDateFrom) AND ewh.[Enabled] = 1
