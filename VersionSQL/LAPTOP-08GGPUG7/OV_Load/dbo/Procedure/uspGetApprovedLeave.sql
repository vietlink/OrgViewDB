/****** Object:  Procedure [dbo].[uspGetApprovedLeave]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- Description:	
-- =============================================
CREATE PROCEDURE [dbo].[uspGetApprovedLeave] 
...
AS
BEGIN
	-- SET NOCOUNT ON added to prevent extra result sets fromQ
	...
	WHERE
		r.IsCancelled = 0
		and e.status<>'Deleted' AND
		--ewh.EmployeeWorkHoursHeaderID = r.EmployeeWorkHoursHeaderID AND
		--((ew.DateFrom<=@dateTo  and ew.DateTo>=@dateFrom) or (ew.DateFrom<=@dateTo and ew.DateTo is null)) and 
