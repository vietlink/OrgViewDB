/****** Object:  Procedure [dbo].[uspAddUpdateOrgChartEntryGroup]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[uspAddUpdateOrgChartEntryGroup](@id int, @description varchar(50), @code varchar(10), @oldCode varchar(10), @isAddGroup bit)
AS
BEGIN
	...
	ELSE
	BEGIN
		INSERT INTO OrgChartEntryGroups([description], code, IsAddGroup, sortorder)
			VALUES(@description, @code, @isAddGroup, (SELECT ISNULL(MAX(sortorder), 0) + 1 FROM OrgChartEntryGroups))
		SET @retId = @@IDENTITY;

		DECLARE @entityId int = 0;
		SELECT @entityId = id FROM Entity WHERE tablename = 'OrgChartEntryGroups'
