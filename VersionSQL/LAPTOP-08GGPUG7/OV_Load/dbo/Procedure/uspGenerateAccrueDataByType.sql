/****** Object:  Procedure [dbo].[uspGenerateAccrueDataByType]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[uspGenerateAccrueDataByType](@currentDate datetime, @empId int, @accrueInAdvancedMode bit, @ltId int)
AS
BEGIN
	...
	SET @headerId = dbo.fnGetWorkHourHeaderIDByDay(@empId, @currentDate);

	DECLARE @leaveClassify int;
	SELECT @leaveClassify = LeaveClassify FROM LeaveType WHERE ID = @ltId;
	...
	DECLARE leaveTypeCursor CURSOR  
		...
		WHERE lt.AccrueLeave = 1 AND elt.[Enabled] = 1 AND elt.EmployeeID  = @empId AND elt.EmployeeWorkHoursHeaderID = @headerId AND lt.AccrueInAdvanced = @accrueInAdvancedMode AND lt.LeaveClassify = @leaveClassify
	...
	BEGIN
		...
		DECLARE @balance decimal(25, 15) = dbo.fnGetTotalAccrualCount2(@currentDate, @empId, @leaveTypeId, 0);

		DECLARE @currentBalance decimal(25,15);-- = dbo.fnGetTotalAccrualCount2(GETDATE(), @empId, @leaveTypeId);
